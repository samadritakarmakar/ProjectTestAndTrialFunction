...
public:
    ...
    void RunSystemAssembly(std::shared_ptr<GenericLocalIntegrator>& Integrate, sp_mat& A)
    {
    ...
    void RunSystemAssemblyVector(std::shared_ptr<GenericLocalIntegrator>& Integrate, mat& b)
    {
        SetLocalIntegrator(Integrate);
        NodePositions=std::vector<umat>(u_Internal.NoOfElementTypes);
        for (int ElmntTyp=0; ElmntTyp<u_Internal.NoOfElementTypes; ElmntTyp++)
...
private:
    ...
    std::shared_ptr<LocalIntegrator<GenericTrialFunction>> integrate;
    std::vector<umat> NodePositions;
    sp_mat* A_Internal;
    mat * b_Internal;

    inline sp_mat BatchFill_Atemp(sp_mat& A, int& ElmntTyp, umat& positions)
    {
        //Configuration for Batch addition of matrix to global matrix
        bool add_values=false;
        bool sort_locations=true;
